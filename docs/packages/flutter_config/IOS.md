...
> Project update (Auto-Truck): for the latest project-specific setup, read `NEW_PROJECT_SETUP.md`.
> Current project uses env-driven app display name:
> - `Info.plist` -> `CFBundleDisplayName = $(APP_NAME)`
> - `Debug.xcconfig` and `Release.xcconfig` both include `#include? "tmp.xcconfig"`.
