...
- New project bootstrap guide (recommended): `NEW_PROJECT_SETUP.md`
- Android reference (legacy/general): `ANDROID.md`
- iOS reference (legacy/general): `IOS.md`

Current Auto-Truck configuration:

- `PACKAGE_NAME=vn.x9labs.tuktuk`
- `BUNDLE_ID=vn.x9labs.tuktuk`
- Android app label reads from `.env` key `APP_NAME`
- iOS display name reads from `.env` key `APP_NAME`
